...
Uso:
  ./scripts/deploy/predeploy.sh          (recomendado — usa venv automaticamente)
  python3 scripts/deploy/predeploy.py    (se venv já ativo)
...
OUT = PROJECT_ROOT / "docs" / "deploy" / "pre-deploy-tests" / \
...
> **Gerado automaticamente por** `./scripts/deploy/predeploy.sh` em {DATE} {TIME}
