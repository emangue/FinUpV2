...
Este módulo contém a lógica de deduplicação de transações,
comparando com dados existentes em journal_entries e base_parcelas.

Estratégias de Deduplicação:
1. IdTransacao exato (hash FNV-1a 64-bit completo)
2. Base de Parcelas (verifica qtd_pagas para transações parceladas)
3. Data + Valor + Estabelecimento similar (para extratos com variação de nome)

Tabela Temporária:
- duplicados_temp: Armazena duplicados detectados durante upload

Funções Principais:
- deduplicate_transactions(): Deduplica lista de transações
- get_duplicados_temp(): Recupera duplicados temporários
- clear_duplicados_temp(): Limpa tabela de duplicados temporários
- get_duplicados_count(): Conta duplicados temporários

IMPORTANTE: 
- Este código depende de models.py (JournalEntry, DuplicadoTemp, BaseParcelas)
