...
export async function GET(request: NextRequest) {
  try {
    ...
    // Filtros avançados
    const categoriaGeral = searchParams.get('categoriaGeral')
    ...
    if (categoriaGeral && categoriaGeral.trim()) {
      whereClauses.push('CategoriaGeral = ?')
      params.push(categoriaGeral.trim())
    }
    ...
    // Buscar soma dos valores (com sinal)
    const sumQuery = `SELECT SUM(Valor) as somaTotal FROM journal_entries ${whereClause}`
    const sumResult = db.prepare(sumQuery).get(...params) as { somaTotal: number }
    const somaTotal = sumResult.somaTotal || 0
