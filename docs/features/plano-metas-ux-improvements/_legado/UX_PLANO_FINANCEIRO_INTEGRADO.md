...
## Tema 5 — Gestão de Usuários no app_admin

### Visão geral

O `app_admin` é o único ponto de criação de contas — não há auto-cadastro no app principal. A interface deve dar ao admin controle total sobre o ciclo de vida de cada usuário, com fricção proporcional ao risco da ação.

**Hierarquia de risco → design:**
- 🟢 **Baixo risco** (criar, editar, reativar): botões outline, sem confirmação extra
- 🟡 **Risco médio** (desativar): botão outline vermelho, confirmação simples
- 🔴 **Alto risco** (purge total): botão sólido vermelho, modal de **2 etapas obrigatórias**

---

### Tabela de contas com toggle de inativos
...
**Coluna "Dados":** carregada em paralelo (skeleton loading); tooltip ao hover: `N uploads · N grupos · tem plano · tem investimentos`

**Toggle "Ver inativos":** quando ativo, exibe linhas esmaecidas com badge "INATIVA"; contador no header atualiza para `4 ativas + 1 inativa`

**Botão Purge:** nunca é a primeira ação exibida; sempre aparece após Desativar/Reativar
...
│  Senha inicial   [____________________________]  │
│                  ☑ Exigir troca no 1.º login     │
│                                                  │
│           [Cancelar]  [Criar Conta →]            │
└──────────────────────────────────────────────────┘
```

**Após criar:** toast "Conta criada. Grupos padrão e perfil financeiro configurados automaticamente." — trigger de inicialização no backend, transparente para o admin.

---

### Modal de purge — 2 etapas obrigatórias

**Etapa 1 — Resumo de impacto:**

```
┌─────────────────────────────────────────────────────────┐
│  ⚠️  Excluir permanentemente: João Silva            [X] │
├─────────────────────────────────────────────────────────┤
│  Ação IRREVERSÍVEL. Todos os dados serão removidos:     │
│                                                         │
│    🗂  1.204 transações       📤 23 uploads             │
│    📋 18 grupos configurados  📈 Tem investimentos      │
│    📊 Tem plano financeiro                              │
│                                                         │
│  ┌─────────────────────────────────────────────────┐   │
│  │  ⚠️  Usuário tem investimentos vinculados.      │   │
│  │  Esses vínculos também serão removidos.          │   │
│  └─────────────────────────────────────────────────┘   │
│                                                         │
│              [Cancelar]  [Continuar →]                  │
└─────────────────────────────────────────────────────────┘
...
**Etapa 2 — Confirmação por e-mail:**

```
┌─────────────────────────────────────────────────────────┐
│  ⚠️  Confirmar exclusão permanente                  [X] │
├─────────────────────────────────────────────────────────┤
...
│                                                         │
│       [← Voltar]  [🗑 Excluir Permanentemente]         │
│                    (cinza até e-mail bater exato)       │
└─────────────────────────────────────────────────────────┘
```

---

## Tema 6 — Histórico de Uploads e Rollback

### Tela `/mobile/uploads` — Histórico de uploads

Acessível via ⚙️ Perfil → "Meus Uploads" (nova entrada no menu de perfil).

```
┌──────────────────────────────────────────────────────────────────────┐
│  Meus Uploads                                            [← Voltar]  │
├──────────┬─────────────┬─────────────────┬──────┬────────────────────┤
│  Banco   │  Período    │  Enviado em     │  Tx  │  Status            │
├──────────┼─────────────┼─────────────────┼──────┼────────────────────┤
│  Itaú    │  Jan/2026   │  15/01  14:32   │  87  │  ✅ Concluído [↩] │
│  BTG     │  Jan/2026   │  14/01  09:11   │  34  │  ✅ Concluído [↩] │
│  Itaú    │  Dez/2025   │  03/01  18:44   │  91  │  ✅ Concluído [↩] │
│  Itaú    │  Nov/2025   │  05/12  10:20   │  78  │  ↩️ Revertido     │
│  BTG     │  Nov/2025   │  04/12  08:55   │  29  │  ✅ Concluído [↩] │
└──────────┴─────────────┴─────────────────┴──────┴────────────────────┘
```

**Regras de UX:**
- Ordem: mais recente primeiro; paginação de 20 por página
- Badge de status: 🟢 Concluído · ⚫ Revertido · 🟡 Processando · 🔴 Erro (tooltip com motivo)
- Botão `[↩]` apenas em linhas com status **Concluído**; linha Revertida esmaecida, sem ação
- Linha com Erro: sem botão de rollback (dados podem estar parcialmente gravados — exibir mensagem de contato)

---

### Modal de rollback

**Caso padrão (sem vínculos de investimento):**

```
┌────────────────────────────────────────────────────────┐
│  ↩️  Desfazer upload: Itaú · Dez/2025             [X] │
├────────────────────────────────────────────────────────┤
│  Isso vai remover permanentemente:                     │
│                                                        │
│    📄 91 transações deste upload                      │
│    🏷  12 classificações aprendidas por este upload    │
│         (criadas manualmente não são afetadas)         │
│    📅  0 parcelas previstas associadas                │
│                                                        │
│  Os demais uploads e transações não são afetados.      │
│                                                        │
│            [Cancelar]  [Confirmar desfazer →]          │
└────────────────────────────────────────────────────────┘
```

**Com vínculo de investimento ativo:**

```
┌────────────────────────────────────────────────────────┐
│  ↩️  Desfazer upload: BTG · Jan/2026              [X] │
├────────────────────────────────────────────────────────┤
│  Isso vai remover permanentemente:                     │
│    📄 34 transações      🏷  4 classificações          │
│                                                        │
│  ┌──────────────────────────────────────────────────┐ │
│  │  ⚠️  3 transações têm vínculos de investimento  │ │
│  │  ativos. Esses vínculos também serão removidos   │ │
│  │  do portfólio ao desfazer este upload.           │ │
│  └──────────────────────────────────────────────────┘ │
│                                                        │
│      [Cancelar]  [Desfazer e remover vínculos →]       │
└────────────────────────────────────────────────────────┘
```

**Após rollback concluído:**
- Toast: "Upload desfeito. 91 transações removidas."
- Linha muda de "✅ Concluído" para "↩️ Revertido" na tabela (sem recarregar a página)
- Botão `[↩]` desaparece; o registro permanece para auditoria
