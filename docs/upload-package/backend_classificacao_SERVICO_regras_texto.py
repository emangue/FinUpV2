...
class GenericClassificationService:
    ...
    def get_grupos_subgrupos_com_tipos(self, user_id: int) -> List[Dict[str, Any]]:
        """Retorna grupos/subgrupos com tipos de gasto baseado na base_grupos_config do usuário"""
        from app.domains.transactions.models import JournalEntry
        from app.domains.grupos.models import BaseGruposConfig
        
        # Busca combinações únicas de grupo/subgrupo das transações do usuário
        ...
        # Mapear grupos → tipos de gasto da base_grupos_config do usuário
        grupos_config = self.db.query(BaseGruposConfig).filter(BaseGruposConfig.user_id == user_id).all()
